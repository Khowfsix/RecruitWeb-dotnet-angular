...
export class AddFormComponent {
  ...
  public observableCategoryPositions = this.categoryPositionService.getAllCategoryPositions();
  ...
  public fetchRecruiterInfor(userId: string | undefined) {
    ...
    if (userId) {
      this.recruiterService.getRecruiterByUserId(userId)
        .subscribe({
          next: (data) => {
  ...
  private fetchAllLanguages() {
    this.languageService.getAllLanguagues()
      .subscribe({
        next: (data) => {
  ...
  ngOnInit(): void {
    ...
    combineLatest([this.observableLanguages, this.observableCategoryPositions]).subscribe(([languages, categoryPositions]) => {
  ...
  private setupValidators() {
    this.addForm.get('languageName')?.setValidators([Validators.required, this.isInAllowedValues(this.fetchLanguages.map(x => x.languageName))]);
    this.addForm.get('categoryPositionName')?.setValidators([Validators.required, this.isInAllowedValues(this.fetchCategoryPositions.map(x => x.categoryPositionName))]);
  ...
  public addForm: FormGroup = this.formBuilder.group({
    ...
    maxHiringQty: [null, [Validators.required, Validators.min(1), Validators.pattern("^[0-9]*$")]],
  ...
  private isInAllowedValues(allowedValues: any[]): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return allowedValues.includes(value) ? null : { invalid: 'invalid value' };
    }
  }
  ...
  public aaa() {
    ...
    this.toastr.success('Position added...', 'Successfully!', { timeOut: 2000 });
  }
  ...
  public savePosition() {
    ...
    formValue = {
      ...
      languageId: this.fetchLanguages?.find(x => x.languageName === this.addForm?.get('languageName')?.value)?.languageId,
      categoryPositionId: this.fetchCategoryPositions?.find(x => x.categoryPositionName === this.addForm?.get('categoryPositionName')?.value)?.categoryPositionId,
    };
