...
export class ApplicationService {
	...
	createApplication(
		newApplication: ApplicationAddModel,
	...
	updateStatusApplication(
		...
		let url =
			'/api/Application/UpdateStatusApplication/' + applicationId + '?';
		if (companyStatus) {
	...
	getAllByPositionId(
		...
		if (sortString) url = url + '&sortString=' + sortString;
		if (notInBlackList) url = url + '&notInBlackList=' + notInBlackList;
		if (candidateStatus) url = url + '&candidateStatus=' + candidateStatus;
		if (companyStatus) url = url + '&companyStatus=' + companyStatus;
	...
	getApplicationsOfCandidate(candidateId: string) {
		return this.api.GET(
			'/api/Application/GetApplicationsOfCandidate/' + candidateId,
		);
	}
}
